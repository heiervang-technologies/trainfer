# trainfer — submission writeup

Hackathon build of the trainfer daemon spec in `PLAN.md`, built in one session on a single RTX 3090 (24 GB). Read `DESIGN.md` for load-bearing choices and `STATUS.md` for the honest accounting of what works.
...
python -m trainfer.tests.test_residual_live_path && \
python -m trainfer.tests.test_span_prefix && \
python -m trainfer.tests.test_replay && \
