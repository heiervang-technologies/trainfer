"""CLI argument parsing for ``python -m trainfer.server``.

Lives outside ``trainfer.server`` so it can be imported (and tested) without
...
def parse_cli_args(argv: list[str] | None = None) -> ServeConfig:
    ...
    cfg = ServeConfig()
    p = argparse.ArgumentParser(prog="trainfer.server")
