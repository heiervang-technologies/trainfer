...
Terms that appear in trainfer code comments and docs but are not standard
...
— Cleo's razin-safety-sharpened.md (in `cont/docs/research/proofs/`) characterizes
...
`cont/docs/research/proofs/unlike-kl-step-size-bound.md` (A rev3) and
...
- `Φ_obs := Σ_i TV_sim^{emp}_i` — cumulative session drift, accumulated across
  ...
  boundaries. Reset-on-resume matches trainfer's default `disable_adapter()` anchor
...
what `trainfer/objectives/unlike.py`'s tiered preconditions carry at dispatch time
(Tier 4 per-step, Tier 5 cumulative).

### Why it matters for trainfer

trainfer's live-learning loop ingests small, noisy feedback batches and
...
- Ethayarajh, K., Xu, W., Muennighoff, N., Jurafsky, D., Kiela, D.
  ...
  (Unary, desirable/undesirable objective used in trainfer for binary
