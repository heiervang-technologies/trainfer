# trainfer (`trainfer`) — Design & Development Plan

**Status:** v0.1 draft, living document
**Codename:** trainfer / `trainfer` (CLI + pip package)
...
```bash
# Start the daemon
trainfer serve --model qwen3-8b --adapter live --port 8000
...
2. **`trainfer`** (the new repo): the daemon, API, adapter manager, trajectory store, control plane. Imports `ht-unsloth` as a pinned dependency. This is ~80% of the actual product code.

Rule of thumb: **if a change could plausibly be upstreamed to Unsloth, it belongs in `ht-unsloth`. Otherwise it belongs in `trainfer`.** This keeps the fork lean and makes upstream rebases tractable.
...
### 5b.3 The unified feedback pipeline for trainfer
...
*This section is speculative research design rather than a settled plan — but it's mathematically grounded and aligned with 2025–2026 directions. If it works, it's trainfer's novel contribution. If parts don't work, they're additive to the CoH/KTO baselines.*
...
**If it works**, this becomes trainfer's headline capability: "send a critique, get a learned correction, measurable in one update." If it doesn't — specifically if the π-is-its-own-judge assumption fails for small models — we fall back to CoH + KTO, which are already robust.
...
Legend: 🆕 = novel-to-trainfer contribution. 📖 = adapted from cited literature. 🔧 = engineering composition of known parts.
...
- `trainfer.server` — FastAPI app, routes, auth, streaming
- `trainfer.engine.inference` — generation path (Unsloth `fast_generate` or vLLM sidecar)
- `trainfer.engine.train` — wraps ht-unsloth trainers; implements the objective composer
- `trainfer.state` — the live model state (base + merged_deltas + active_adapter)
- `trainfer.queue` — compute buffer, chunking, commit tokens
- `trainfer.adapters` — LoRA pool management, hot-swap, progressive merge
- `trainfer.trajectory` — append-only log, replay, snapshot reconstruction
- `trainfer.snapshot` — save/restore/diff
- `trainfer.objectives` — SFT, PG, GRPO, KL, distillation, prompt-baking; pluggable registry
- `trainfer.controller` — the single writer serializing train/merge/save/load on the GPU
...
6. **Packaging.** `pip install trainfer` with GPU-detect auto-install? Docker image? Both?
...
- `trainfer serve` stands up FastAPI + Unsloth fast_generate + a single LoRA adapter.
...
1. `pip install trainfer && trainfer serve --model qwen3-8b` just works on a single 3090.
...
This is the single highest-leverage uncertainty in the plan. If rankings are reliable, CCPD v2 is real and trainfer has a defensible innovation story. If rankings are noise at our target model scales (7B-14B), we fall back to a pure T1 + T4 system, which works but doesn't have a headline feature.
