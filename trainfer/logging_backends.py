...
The JSONL trajectory at ``trainfer_data/trajectory.jsonl`` is the canonical
...
do not break trainfer; the adapter raises at construction, and ``get_logger``
...
# ---------------------------------------------------------------------- null
class NullLogger:
    """No-op sink. Selected by default so trainfer has zero external deps
...
class TensorBoardLogger:
    ...
    def log_params(self, params: Mapping[str, Any]) -> None:
        ...
        try:
            ...
            self._writer.add_text("trainfer/params", text, global_step=0)
        except Exception as exc:  # pragma: no cover
