...
class ChatRequest(BaseModel):
    ...
    # Lets a client pick "trainfer with LoRA" vs "trainfer base" at request time
...
class MemorizeRequest(BaseModel):
    ...
    Drives the greedy-memorize loop in ``trainfer.memorize``. Threshold is the
...
def create_app(cfg: ServeConfig | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        # Out-of-tree verifiers (``trainfer.verifiers`` entry-point group).
        # Benchmark-shaped verifiers ship their own corpora and live outside
        # the daemon — see ``objectives/verifiers.load_plugins``. Never fatal.
        from .objectives.verifiers import load_plugins as _load_verifier_plugins

        _load_verifier_plugins()
        ...
        # Gated on cfg.dev_autoreload (or TRAINFER_DEV_AUTORELOAD=1). Safe to
        # call without jurigged installed — logs a warning and proceeds.
        if cfg.dev_autoreload or os.environ.get("TRAINFER_DEV_AUTORELOAD") == "1":
    ...
    app = FastAPI(title="trainfer", version="0.1.0-dev", lifespan=lifespan)
    app.state.cfg = cfg
                ...
                try:
                    async for ev in c.stream_generate(
                        ...
                        payload = {
                            ...
                            "trainfer": {"response_id": ev["response_id"]},
                        }
                        yield f"data: {json.dumps(payload)}\n\n"
    ...
    async def commits_stream():
        """Per-commit event stream; see cont/docs/research/pr-specs/commits-sse-stream.md.
