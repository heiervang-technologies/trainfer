...
``trainfer/objectives/ttrl_mv.py`` as the module path, but architecturally this
is a *scheduler* — the rollout-sampling step must run in inference mode,
outside the queue worker — so it sits at package top level next to
:class:`IdleReplayScheduler` rather than under ``objectives/``. The training
signal itself still flows through the existing ``sft`` objective; TTRL is a
pseudo-label generator, not a new loss.
...
harness task by >5pp; kill if it does" — requires the ``cont`` eval
...
bound from ``trainfer/GLOSSARY.md``). The verifier filter ensures we only
