"""HTTP smoke test for the trainfer FastAPI server.
...
Run with: python -m trainfer.tests.smoke_server
"""
...
def main() -> int:
    port = _pick_port()
    data_dir = pathlib.Path(tempfile.mkdtemp(prefix="trainfer_smoke_"))
    cfg = ServeConfig(
    ...
    with httpx.Client(base_url=base, timeout=180.0) as c:
        ...
        cursor = body["trainfer"]["commit_cursor"]
