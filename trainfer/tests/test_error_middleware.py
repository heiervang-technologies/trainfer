...
Run with: pytest trainfer/tests/test_error_middleware.py
"""
...
def _build_app() -> FastAPI:
    ...
    from trainfer.middleware import RequestIDMiddleware, current_request_id
...
def test_trajectory_stamps_request_id_when_set(tmp_path):
    from trainfer.middleware import _REQUEST_ID_CTX, current_request_id, set_request_id
    from trainfer.trajectory import TrajectoryLog
...
def test_log_records_carry_request_id(caplog):
    from trainfer.middleware import (
        RequestIdLogFilter,
        _REQUEST_ID_CTX,
        set_request_id,
    )

    logger = logging.getLogger("trainfer.test_rid")
    ...
    try:
        with caplog.at_level(logging.INFO, logger="trainfer.test_rid"):
