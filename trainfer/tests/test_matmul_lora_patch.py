"""Unit tests for ``trainfer.state._install_matmul_lora_patch`` against a stub
``unsloth`` package.

The patch installer is the single load-bearing coupling between trainfer and the
ht-unsloth fork; ``trainfer/state.py:55-77``'s signature guard is what stops a
...
def _make_stub_matmul_lora():
    ...
    signature trainfer expects. Computes a deterministic ``X @ W.T`` and ignores
...
def _fresh_install_state():
    """``trainfer.state`` caches its install via ``functools.cache``. Clear the
    cache so tests can drive the install function independently of any
    earlier-test side effects."""
    from trainfer import state as state_mod
...
def test_signature_match_installs_wrapper(monkeypatch):
    ...
    assert getattr(utils.matmul_lora, "_trainfer_patched", False) is True
    assert utils.matmul_lora._trainfer_original is original
...
def test_signature_mismatch_skips_install(monkeypatch, caplog):
    ...
    with caplog.at_level("WARNING", logger="trainfer.state"):
        state_mod._install_matmul_lora_patch()
...
def test_install_is_idempotent(monkeypatch):
    ...
    a second wrap would lose ``_trainfer_original`` and keep stacking forwards."""
    original = _make_stub_matmul_lora()
    ...
    assert second._trainfer_original is original
...
def test_whitelist_excludes_this_file():
    ...
    import trainfer.tests.conftest as conftest_mod
    ...
    assert Path(__file__).name not in conftest_mod._TORCHLESS_OK, (
        ...
        "in _TORCHLESS_OK. Remove it from the whitelist in trainfer/tests/conftest.py."
    )
