...
``cont/docs/research/pr-specs/unlike-tiered-preconditions.md``:
...
from trainfer.objectives.unlike import (
...
def test_pure_unlike_target_position_scope_no_warn() -> None:
    ...
    with warnings.catch_warnings(record=True) as rec:
        ...
        unlike_loss(
            ...
            samples=[{"prefix": "abc", "bad_token_id": 7, "rank_below": 5}],
            batch_objectives=[
                {"name": "kl_anchor", "scope": "target_position", "weight": 0.1},
            ],
            effective_lr=5e-4,
        )
    tier_warns = [
...
def test_positive_teacher_no_anchor_no_error() -> None:
    ...
    out = unlike_loss(
        ...
        tok,
        samples=[
            {"prefix": "abc", "bad_token_id": 7, "good_token_id": 11, "rank_below": 5}
        ],
        batch_objectives=[],
        effective_lr=5e-4,
    )
...
def test_bare_primitive_call_runs_without_preconditions() -> None:
    ...
    assert "loss" in out
    tier_warns = [w for w in rec if issubclass(w.category, RuntimeWarning)]
    assert tier_warns == [], [str(w.message) for w in tier_warns]
