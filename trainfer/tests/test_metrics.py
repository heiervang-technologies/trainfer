...
Run with: pytest trainfer/tests/test_metrics.py
"""

from __future__ import annotations


# NOTE: these tests lazy-import ``trainfer.metrics`` inside their bodies, which
...
def test_render_prometheus_returns_bytes_with_text_format_header():
    ...
    assert "# HELP trainfer_requests_total" in text
    assert "# TYPE trainfer_requests_total counter" in text
    assert "# HELP trainfer_queue_depth" in text
    assert "# TYPE trainfer_queue_depth gauge" in text
...
def test_record_train_step_increments_counter_and_histogram():
    ...
    before = _counter_value(
        metrics.render_prometheus(), "trainfer_train_steps_total", {"objective": "sft"}
    )
    metrics.record_train_step(objective="sft", latency_s=0.123, loss=1.5)
    text = metrics.render_prometheus().decode("utf-8")
    after = _counter_value(
        text.encode(), "trainfer_train_steps_total", {"objective": "sft"}
    )
    assert after == before + 1.0
    # Histogram count bumped.
    assert "trainfer_step_latency_seconds_count" in text
...
def test_record_feedback_event_counter():
    ...
    before = _counter_value(
        metrics.render_prometheus(), "trainfer_feedback_events_total", {"kind": "binary"}
    )
...
def test_record_generate_latency_observes_histogram():
    ...
    text = metrics.render_prometheus().decode("utf-8")
    assert 'trainfer_generate_latency_seconds_count{stream="false"}' in text
    assert 'trainfer_generate_latency_seconds_count{stream="true"}' in text
...
def test_gauge_collector_samples_live_state(tmp_path):
    ...
    assert _sample_value(text, "trainfer_queue_depth") == 3.0
    assert _sample_value(text, "trainfer_commit_cursor") == 42.0
    assert _sample_value(text, "trainfer_merges_applied") == 7.0
    assert _sample_value(text, "trainfer_trajectory_bytes") == 1024.0
    assert _sample_value(text, "trainfer_snapshots_count") == 2.0
    assert _sample_value(text, "trainfer_snapshots_bytes") == 100.0
    assert _sample_value(text, "trainfer_shutting_down") == 0.0
...
def test_shutting_down_gauge_flips_when_flag_set(tmp_path):
    ...
    metrics.bind_controller(ctrl)
    text = metrics.render_prometheus().decode("utf-8")
    assert _sample_value(text, "trainfer_shutting_down") == 1.0
...
def test_unbound_gauges_emit_zero(tmp_path):
    ...
    assert _sample_value(text, "trainfer_queue_depth") == 0.0
    assert _sample_value(text, "trainfer_shutting_down") == 0.0
...
def test_metrics_route_emits_prom_text_format():
    ...
    from trainfer.server_errors import register_error_handlers
    ...
    assert "trainfer_requests_total" in r.text
...
def test_openmetrics_negotiation_returns_exemplar_content_type():
    ...
    assert b"# HELP trainfer_requests_total" in plain_body
...
def test_exemplar_attached_when_request_id_bound():
    ...
    from trainfer.middleware import _REQUEST_ID_CTX, set_request_id
    from trainfer import metrics
...
def test_metrics_route_wired_in_server():
    ...
    from trainfer.server import create_app

    cfg = ServeConfig(data_dir=pathlib.Path(tempfile.mkdtemp(prefix="trainfer_m_")))
    app = create_app(cfg)
