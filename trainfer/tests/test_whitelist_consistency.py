...
def _load_conftest_module():
    """Import ``trainfer/tests/conftest.py`` as a plain module.
    ...
    spec = importlib.util.spec_from_file_location(
        "_trainfer_conftest_under_test", here / "conftest.py"
    )
...
def test_live_whitelist_is_consistent():
    """The real ``trainfer/tests/`` tree must have no cpu_only misses.
    ...
    # When run with torch present, files that import ``trainfer.controller``
