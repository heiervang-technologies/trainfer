...
``cont/docs/research/pr-specs/commits-sse-stream.md``:
...
torch-free: imports ``trainfer.commit_stream`` only, then builds a minimal
FastAPI app inline that mounts the same stream-handler logic the real
``/v1/commits/stream`` route uses in ``trainfer/server.py``.
"""
...
from trainfer.commit_stream import CommitBroadcaster, _iso_now_ms
...
def _make_app(broadcaster: CommitBroadcaster, *, keepalive_s: float = 15.0) -> FastAPI:
    """Mount the same stream-handler logic that ``trainfer.server`` exposes.
