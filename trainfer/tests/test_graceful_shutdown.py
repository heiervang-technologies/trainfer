...
- flips ``_shutting_down`` (read by ``metrics.py`` for the ``trainfer_shutting_down``
  gauge),
...
# import ``trainfer.controller`` / ``trainfer.server`` inside their bodies,
...
def test_submit_train_rejected_after_shutdown(tmp_path):
    async def main():
        from trainfer.errors import ShuttingDownError
...
def test_in_flight_pending_tasks_resolve_with_shutdown_dropped(tmp_path):
    ...
    async def main():
        from trainfer.errors import ShutdownDroppedError
