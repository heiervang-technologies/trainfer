...
Runs a FastAPI app with the same lifespan wiring as :mod:`trainfer.server` but
...
- ``TRAINFER_SIGTERM_SENTINEL`` — path to write on graceful_shutdown
- ``TRAINFER_SIGTERM_PORT`` — port to bind
- ``TRAINFER_SIGTERM_DEADLINE`` — ``shutdown_deadline_s`` to pass (default 5.0)
- ``TRAINFER_SIGTERM_GRACE`` — ``shutdown_hard_stop_grace_s`` to pass (default 5.0)
"""
...
def main() -> None:
    sentinel = Path(os.environ["TRAINFER_SIGTERM_SENTINEL"])
    port = int(os.environ["TRAINFER_SIGTERM_PORT"])
    deadline = float(os.environ.get("TRAINFER_SIGTERM_DEADLINE", "5.0"))
    hard_stop_grace = float(os.environ.get("TRAINFER_SIGTERM_GRACE", "5.0"))
