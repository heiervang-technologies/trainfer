...
Covers the pure-function ``_should_trigger`` in trainfer/objectives/unlike.py.
The full end-to-end ``unlike_loss`` requires a model+tokenizer and is
covered by smoke_objectives.

Run: python -m trainfer.tests.test_unlike_trigger  (or pytest)
"""
...
from trainfer.objectives.unlike import _should_trigger
