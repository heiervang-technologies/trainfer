...
from trainfer.objectives.ccd import ccd_loss
...
def test_ccd_loss_forward_and_backward():
    ...
    from trainfer.objectives.verifiers.fact_verifier import verify_facts

    retention_sum = 0.0
    for prompt, facts in probes:
        s_ids = state.tokenizer(prompt, return_tensors="pt").input_ids.to(
            state.model.device
        )
    ...
    assert retention_rate >= 0.80, (
        f"fact retention failed, got {retention_rate:.2f} < 0.80"
    )
