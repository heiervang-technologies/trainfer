...
def test_sigterm_fires_graceful_shutdown(tmp_path):
    ...
    env = dict(
        os.environ,
        TRAINFER_SIGTERM_SENTINEL=str(sentinel),
        TRAINFER_SIGTERM_PORT=str(port),
        TRAINFER_SIGTERM_DEADLINE="5.0",
        TRAINFER_SIGTERM_GRACE="3.0",
    )
    proc = subprocess.Popen(
        [sys.executable, "-m", "trainfer.tests._sigterm_target"],
        env=env,
