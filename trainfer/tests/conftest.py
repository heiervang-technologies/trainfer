"""pytest conftest for the trainfer test suite.

Most of this suite imports ``torch`` / ``unsloth`` (directly or through
``trainfer.controller`` / ``trainfer.state``) at module scope. On a CI runner
without those heavy deps we still want the few stdlib-only slices to
collect cleanly, so when torch is missing we fall back to a whitelist of
files known to import without it.
...
# ``trainfer/tests/`` will be skipped at collection time when torch is absent.
_TORCHLESS_OK = {
    "test_errors.py",  # lazy trainfer.errors import inside tests
    "test_error_middleware.py",  # same; also uses FastAPI/TestClient
    "test_logging_backends.py",  # trainfer.logging_backends is lazy on heavy deps
    "test_queue_cursor.py",  # trainfer.queue is pure Python
    "test_queue_graceful_drain.py",  # trainfer.queue drain path, asyncio-only
    "test_reasoning.py",  # trainfer.reasoning is pure Python
    "test_trajectory_tail.py",  # trainfer.trajectory is pure Python
    "test_whitelist_consistency.py",  # self-validation of _TORCHLESS_OK
    "test_commits_sse_stream.py",  # trainfer.commit_stream is pure asyncio + FastAPI
    ...
    *and* import cleanly in the current (torchless) interpreter.
    ...
    for p in sorted(tests_dir.glob("test_*.py")):
        ...
        spec = importlib.util.spec_from_file_location(
            f"_trainfer_whitelist_probe_{p.stem}", p
        )
...
if _missing("torch"):
    ...
    if _violations:
        raise AssertionError(
            "trainfer/tests/conftest.py: _TORCHLESS_OK is out of sync with the "
