...
from trainfer.objectives.ccpd import score_rc
...
def _bench_items() -> list[BenchItem]:
    ...
    items.append(
        BenchItem(
            ...
            oracle=lambda y: (
                ...
                if any(ch.isdigit() for ch in y) and len(y.strip()) < 10
                else 0.0
            ),
            seeds=[
                "60",
    ...
    items.append(
        BenchItem(
            ...
            oracle=lambda y: (
                1.0
                if len(y.strip().split()) == 1
                else 1.0 / max(1, len(y.strip().split()))
            ),
            seeds=[
                "Tokyo.",
                "Tokyo",
    ...
    items.append(
        BenchItem(
            ...
            oracle=lambda y: (
                1.0
                if y.strip().isdigit() and int(y.strip()) in {11, 13, 17, 19}
                else 0.5
                if y.strip().isdigit()
                else 0.0
            ),
            seeds=[
                ...
                "A prime between 10 and 20 is 13.",
            ],
        )
    )
    ...
    items.append(
        BenchItem(
            ...
            seeds=[
                "Apples and bananas.",
                "- Apple\n- Banana",
                "Two fruits are apples and bananas.",
            ],
        )
    )
    ...
    items.append(
        BenchItem(
            ...
            oracle=lambda y: (
                sum(1 for ch in y if ch.isalpha() and ch.isupper())
    ...
    items.append(
        BenchItem(
            ...
            oracle=lambda y: (
                -(y.count("#") + y.count("*") + y.count("- ") + y.count("_"))
            ),
            seeds=[
    ...
    items.append(
        BenchItem(
            ...
            oracle=lambda y: (
                (
                    1.5
                    if "330" in y or "324" in y or "300" in y
                    else 1.0
                    if any(ch.isdigit() for ch in y) and "m" in y.lower()
                    else 0.5
                    if any(ch.isdigit() for ch in y)
                    else 0.0
                )
                - len(y.strip()) * 0.001
            ),
            seeds=[
                ...
                "About 330m.",
                "It's tall.",
                "Around three hundred meters.",
                "Quite tall, hard to say exactly.",
            ],
        )
    )
    ...
    items.append(
        BenchItem(
            ...
            oracle=lambda y: (
                ...
                if any(s in y for s in ["1948", "1946", "194"])
                else 0.3
                if any(ch.isdigit() for ch in y)
                else 0.0
            ),
...
def sample_candidates(
    ...
    if critique:
        messages.append(
            {"role": "system", "content": f"Revise based on this feedback: {critique}"}
        )
...
def main() -> int:
    ...
    parser.add_argument("--output", default="trainfer_data/bench_rc.json")
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        ...
        max_seq_length=1024,
        load_in_4bit=args.model.endswith(("4bit", "bnb-4bit"))
        or "bnb-4bit" in args.model,
        dtype=None,
    )
    FastLanguageModel.for_inference(model)
    print(
        f"[bench] loaded in {time.time() - t_load:.1f}s; VRAM {torch.cuda.memory_allocated() / 1e9:.2f} GB"
    )
    ...
    for ii, item in enumerate(items):
        for rep in range(args.repeats):
            ...
            rc_scores = [
                score_rc(
                    model, tokenizer, item.prompt, c, item.critique, beta=args.beta
                )
                for c in cands
            ]
            ...
            print(
                f"  [{ii + 1}/{len(items)}.{rep}] ρ={rho:+.3f} n={len(cands)} "
    ...
    summary = {
        ...
        "decision": (
            "ship_T2_1_default"
            if mean_rho > 0.5
            else "ship_T2_1_k8_with_hinge_primary"
            if mean_rho >= 0.2
            else "fallback_to_sft_self_refinement"
        ),
