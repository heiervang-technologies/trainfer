"""FastAPI exception handlers that emit the ``trainfer.errors`` envelope.

Call ``register_error_handlers(app)`` once during ``create_app`` to wire:

- ``TrainferError`` subclasses → envelope with their own ``status_code`` / ``code``
...
All envelopes include a ``request_id`` read from the ``trainfer.middleware``
...
from .errors import TrainferError, envelope_payload
...
def register_error_handlers(app: FastAPI) -> None:
    @app.exception_handler(TrainferError)
    async def _trainfer_error_handler(_req: Request, exc: TrainferError) -> JSONResponse:
        return _respond(
