"""Jurigged-backed hot reload for the trainfer daemon.
...
Enable via ``cfg.dev_autoreload=True`` or ``TRAINFER_DEV_AUTORELOAD=1``.
"""
...
# Watch the trainfer package directory. Jurigged's ``to_filter`` treats a
# directory path as "everything under it recursively", which covers
# ``trainfer/server.py`` at the top level *and* ``trainfer/objectives/kl.py``
# nested. A glob like ``trainfer/**/*.py`` would miss the top-level files
# because ``**`` requires at least one intermediate directory.
_DEFAULT_PATTERN = "trainfer/"
...
def enable(pattern: str | None = None) -> bool:
    ...
    pat = pattern or os.environ.get("TRAINFER_AUTORELOAD_PATTERN") or _DEFAULT_PATTERN
    # Resolve relative to the trainfer package parent so the daemon can be launched
    ...
    print(f"[trainfer.dev.autoreload] watching {abs_pattern} (poll=True)", flush=True)
