...
- **Transformers 5.x BatchEncoding shape changes** caught `apply_chat_template(return_tensors="pt")` (now returns a BatchEncoding dict, not a bare tensor). Fixed in `trainfer/objectives/_utils.py::_to_int_list` with tolerant unwrapping, and at every generation site by using `apply_chat_template(tokenize=False)` → string → `tokenizer(text=..., return_tensors="pt")`.
...
python -m trainfer.tests.test_queue_cursor
python -m trainfer.tests.test_trajectory_snapshot

# GPU smoke (Qwen3-0.6B, ~10 s)
python -m trainfer.tests.smoke_objectives

# End-to-end (Qwen3-0.6B, ~90 s total)
python -m trainfer.tests.test_merge_and_e2e
python -m trainfer.tests.smoke_server

# CCPD v2 E2E (Qwen3-0.6B, ~2 min — loads model three times)
python -m trainfer.tests.test_ccpd_e2e

# Concurrent-load invariant (Qwen3-0.6B, ~30 s including load)
python -m trainfer.tests.test_concurrent_load

# §11 ranking benchmark (Qwen3-8B, ~4 min)
python -m trainfer.tests.bench_rc_ranking \
    --model unsloth/Qwen3-8B-unsloth-bnb-4bit \
    --k 6 --repeats 2 --max-new-tokens 80 \
    --output trainfer_data/bench_rc_qwen8b_n20.json

# §11 ranking benchmark matched-k rerun (Qwen3-0.6B, ~2 min)
python -m trainfer.tests.bench_rc_ranking \
    --model unsloth/qwen3-0.6b-unsloth-bnb-4bit \
    --k 6 --repeats 2 \
    --output trainfer_data/bench_rc_qwen06b_n20_k6.json
```
...
| File | Lines | Status |
|---|---|---|
| `trainfer/config.py` | ~50 | done |
| `trainfer/state.py` | ~240 | done; mode_lock added, tested under contention |
| `trainfer/queue.py` | ~150 | done; tested |
| `trainfer/trajectory.py` | ~110 | done; tested |
| `trainfer/snapshot.py` | ~130 | done; tested |
| `trainfer/controller.py` | ~240 | done; E2E + HTTP + concurrent-load tested |
| `trainfer/server.py` | ~180 | done; HTTP smoke passes |
| `trainfer/objectives/_utils.py` | ~130 | done; `use_cache=False` fix in `sequence_logprob` |
| `trainfer/objectives/sft.py` | ~70 | done; tested |
| `trainfer/objectives/kto.py` | ~105 | done; tested (smoke); π_ref via `disable_adapter()` when no external ref is supplied |
| `trainfer/objectives/coh.py` | ~70 | done; tested (smoke) |
| `trainfer/objectives/hinge.py` | ~70 | done; tested (smoke); **primary T2 ship default** |
| `trainfer/objectives/kl.py` | ~65 | done; π_ref via `disable_adapter()` |
| `trainfer/objectives/ccpd.py` | ~250 | done; E2E smoke + π_ref wired; **shipped as per-event opt-in** |
| `trainfer/engine/train.py` | ~160 | done; mode_lock held around forward+backward |
| `trainfer/engine/inference.py` | ~65 | done; mode_lock held around generate |
| `trainfer/tests/*` | nine test modules (12 CCPD/server/concurrent assertions) | all passing locally |
