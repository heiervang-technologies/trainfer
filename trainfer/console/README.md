# trainfer trial — Qwen3.5-9B on 127.0.0.1:8765

Server PID: tail of `/tmp/trainfer-trial/server.log`. Stop with `fuser -k 8765/tcp`.
...
Response includes `trainfer.response_id` and `trainfer.commit_cursor`. Keep the `response_id` — feedback references it.
...
The response body's `trainfer.commit_cursor` will be ≥ N.
