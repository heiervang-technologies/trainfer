...
def hinge_contrastive_loss(
    ...
    **_: Any,
    ...
    chosen_tok = [
        build_chat_inputs(tokenizer, s["prompt"], s["chosen"]) for s in samples
    ]
    rejected_tok = [
        build_chat_inputs(tokenizer, s["prompt"], s["rejected"]) for s in samples
    ]
    pad_id = tokenizer.pad_token_id or tokenizer.eos_token_id or 0
    cbat = pad_and_stack(chosen_tok, pad_id=pad_id)
    rbat = pad_and_stack(rejected_tok, pad_id=pad_id)

    chosen_sum = sequence_logprob(
        model, cbat["input_ids"], cbat["labels"], cbat["attention_mask"]
    )
    rejected_sum = sequence_logprob(
        model, rbat["input_ids"], rbat["labels"], rbat["attention_mask"]
    )
