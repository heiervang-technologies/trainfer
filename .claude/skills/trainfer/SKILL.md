---
name: trainfer
description: Orient yourself before touching `trainfer/` — the trainfer live-training daemon. Use when editing any `trainfer/**` file, `studio/backend/routes/lile.py`, `studio/frontend/src/features/lile/**`, or when debugging commit-cursor / residual-delta / queue behavior. Covers the load-bearing invariants, entry points, how to add objectives / metrics backends, and the anti-patterns that have bitten past contributors.
---

# trainfer — trainfer FYI

`trainfer` is an online-learning daemon that wraps a single Unsloth `FastLanguageModel` with a FastAPI server. **Inference and training share the same model weights in one CUDA context.** Clients `POST /v1/train` with a batch, and the next `POST /v1/chat/completions` can block on a `commit_token` to observe the update.

This skill is the orientation pass. Deep design lives in `trainfer/DESIGN.md`; current test evidence lives in `trainfer/STATUS.md`; terminology lives in `trainfer/GLOSSARY.md`.
...
| File | What it owns |
|---|---|
| `trainfer/server.py` | FastAPI routes (`/v1/chat/completions`, `/v1/train`, `/v1/feedback`, `/v1/state/*`, `/v1/wait`, `/health`). OpenAI-compatible SSE. Thin — delegates to `Controller`. |
| `trainfer/controller.py` | The orchestrator. `generate`, `stream_generate`, `submit_train`, `submit_feedback`, `request_merge`, snapshot save/load. Holds the queue, trajectory, and state. |
| `trainfer/queue.py` | `ComputeQueue` — single-worker FIFO with monotone `commit_cursor`. `wait_for(token)` is the "POST a batch, next inference sees it" guarantee. |
| `trainfer/state.py` | `ModelState` — the model, tokenizer, `mode_lock`, `merged_deltas` residual, and the **module-level monkey-patch on `unsloth.kernels.utils.matmul_lora`** that applies `_residual_delta` at forward time. |
| `trainfer/engine/train.py` | `TrainEngine.step` — composes registered objectives, runs forward+backward, updates LoRA, returns `{loss, components}`. |
| `trainfer/engine/inference.py` | `generate_chat` / `generate_chat_stream` — uses HF `TextIteratorStreamer`; holds `mode_lock` across the Unsloth mode flip and the entire generate. |
| `trainfer/engine/replay.py` | `IdleReplayScheduler` — optional background re-training from the trajectory when the queue is idle. |
| `trainfer/objectives/` | One file per objective. All register into `objectives/__init__.py::OBJECTIVES`. |
| `trainfer/reasoning.py` | Streaming `<think>…</think>` parser → splits `reasoning_content` vs `content` deltas. Family registry for Qwen3 / DeepSeek-R1 / Magistral / gpt-oss. |
| `trainfer/logging_backends.py` | Optional no-throw metrics sinks (W&B / TensorBoard / MLflow / trackio) — trajectory JSONL is still the canonical record. |
| `trainfer/trajectory.py` | Append-only JSONL at `trainfer_data/trajectory.jsonl`. Every `train_step`, `inference`, and `feedback` event lands here with a stable byte offset. |
| `trainfer/snapshot.py` | Save/load `merged_deltas` + active LoRA adapter to disk. `residual_fingerprint()` is what tests use to prove byte-equality. |

## The concepts you must hold in your head

**Residual delta.** Upstream Unsloth bypasses both `LoraLayer.forward` and `base_layer.forward` on Qwen3's fast path, so `register_forward_hook` silently never fires. We intercept the single funnel `unsloth.kernels.utils.matmul_lora` via a module-level monkey-patch installed at import of `trainfer/state.py`. The patch looks for `W._residual_delta` (a bf16 GPU tensor bound as an attribute of the Parameter) and adds `F.linear(X, delta)` to the output. Binding by attribute survives Unsloth's mode flips — `id(W)` stays stable across `for_training` / `for_inference`. Do not replace this with a `dict[id(W), delta]` lookup; that was tried and broke on mode flip.
...
**Trajectory as source of truth.** `trainfer_data/trajectory.jsonl` is canonical. Metrics backends in `logging_backends.py` are optional *mirrors* that must never raise into the hot path — catch-and-log inside every adapter method.
...
1. Create `trainfer/objectives/<name>.py`. Export `<name>_loss(model, batch, **kwargs) -> {"loss": Tensor, "components": dict[str, float]}`. The loss must be scalar and graph-attached; `components` is for logging only.
2. Register in `trainfer/objectives/__init__.py::OBJECTIVES`. If the import is optional (heavy extra deps), wrap in `try/except` like `ccpd_v2`.
3. Add to the Razin-safety table in `GLOSSARY.md`. If not safe, document the anchor / cap that makes it acceptable in-loop.
4. Write a smoke test in `trainfer/tests/smoke_objectives.py` (finite gradients on Qwen3-0.6B).
...
1. Add a class to `trainfer/logging_backends.py` implementing the `MetricsLogger` protocol (`log_params`, `log_metrics`, `close`).
...
1. Route code goes in `trainfer/server.py`. Keep it thin — parse pydantic, dispatch to `Controller`, format response.
...
Lives in `studio/frontend/src/features/trainfer/`. Stores are zustand; polling hooks subscribe to a stable trajectory reference (see `chart-utils.ts` for the hook wrappers — raw selectors return a new array each render and trip `useSyncExternalStore`, causing "Maximum update depth exceeded"). New chart cards should use `useLossSeries` / `useGradNormSeries` / etc. rather than inlining `useTrainferCapsuleStore((s) => …)` with a derived array.

## Running the tests

```bash
# Unit-level smoke (fast, CPU-only where possible)
uv run python -m trainfer.tests.smoke_objectives

# Full HTTP path (requires a GPU with a 4-bit Qwen3-0.6B in HF cache)
uv run python -m trainfer.tests.smoke_server

# Concurrent-load invariant — the one that fails under reordering
uv run pytest trainfer/tests/test_concurrent_load.py -xvs

# CCPD v2 end-to-end
uv run pytest trainfer/tests/test_ccpd_e2e.py -xvs
```

Use `uv`, never `pip`. Every test under `trainfer/tests/` that has `test_` prefix is a pytest; files prefixed `smoke_` are executable scripts.
...
- `trainfer/DESIGN.md` — every load-bearing decision with a rationale.
- `trainfer/STATUS.md` — test-by-test evidence; always check this after a substantive change.
- `trainfer/GLOSSARY.md` — `Razin-safe` and other terms that appear in code comments.
- `trainfer/PLAN.md` — the long-form spec and roadmap (phases 0–6).
- `trainfer/SUBMISSION.md` — the narrative that framed the initial build, if you're reconstructing intent.
