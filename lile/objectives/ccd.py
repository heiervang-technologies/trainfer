    ...
    **_: Any
    ...
    for s in samples:
        ...
        if getattr(tokenizer, "chat_template", None):
            is_processor = (hasattr(tokenizer, "image_processor") or hasattr(tokenizer, "video_processor"))
            c_p = context + "\n\n" + prompt
            user_content_t = [{"type": "text", "text": c_p}] if is_processor else c_p
            messages_t = [{"role": "user", "content": user_content_t}]
            t_ids = _to_int_list(tokenizer.apply_chat_template(messages_t, add_generation_prompt=True, tokenize=True, return_tensors=None))

            user_content_s = [{"type": "text", "text": prompt}] if is_processor else prompt
            messages_s = [{"role": "user", "content": user_content_s}]
            s_ids = _to_int_list(tokenizer.apply_chat_template(messages_s, add_generation_prompt=True, tokenize=True, return_tensors=None))
        else:
            t_ids = _to_int_list(tokenizer(text=context + "\n\n" + prompt, add_special_tokens=False).input_ids)
            s_ids = _to_int_list(tokenizer(text=prompt, add_special_tokens=False).input_ids)
    ...
    with torch.no_grad():
        with model.disable_adapter():
            t_out = model(input_ids=t_ids, attention_mask=t_mask, use_cache=False, output_hidden_states=match_hidden)
            t_logits = t_out.logits
            t_hiddens = t_out.hidden_states if match_hidden else None

    # Student forward (with grad)
    s_out = model(input_ids=s_ids, attention_mask=s_mask, use_cache=False, output_hidden_states=match_hidden)
    ...
    for i in range(B):
        ...
        # KL on logits
        # We predict the NEXT token, so we compare logits at [:-1]
        t_log_p = F.log_softmax(t_logits[i, start_t:L_t-1], dim=-1) # teacher log_p
        s_log_p = F.log_softmax(s_logits[i, 0:L_s-1], dim=-1) # student log_q
